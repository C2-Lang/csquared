 ...
 * @details Implements symbol tables, scoping, and name resolution for semantic
 * analysis. Supports multiple scopes with proper shadowing and symbol lookups.
 */
...
/**
 * @brief Symbol scope.
 * @details Represents a scope level containing a symbol table with hash
 * buckets. Supports nested scopes with parent pointers.
 */
struct sym_scope {
  struct sym_scope *parent; /**< Parent scope */
  struct sym_hash_bucket
      *hash_table;     /**< Hash table for symbols in this scope */
 ...
 * @return Pointer to the symbol if found, NULL otherwise.
 */
struct csq_symbol *sym_lookup_recursive(struct sym_scope *scope,
