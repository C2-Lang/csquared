...
static void strip_att_suffix(char *dest, const char *opcode, size_t size) {
  if (!opcode || !dest || size == 0)
    return;

  size_t len = strlen(opcode);

  if (len > 0 && (opcode[len - 1] == 'l' || opcode[len - 1] == 'w' ||
                  opcode[len - 1] == 'b' || opcode[len - 1] == 'q')) {
...
static void convert_to_intel_syntax(char *intel_operands,
                                    const char *att_operands, size_t size) {
  if (!intel_operands || !att_operands || size == 0)
    return;
