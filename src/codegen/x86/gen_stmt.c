 ...
 * @details Generates code for control flow statements (if, while, for, switch,
 * etc).
 */
...
static bool gen_push_loop(gen_ctx *ctx, const char *start_label,
  ...
  if (ctx->loop_stack_count >= ctx->loop_stack_capacity) {
    ...
    loop_label *new_stack =
        realloc(ctx->loop_stack, sizeof(loop_label) * new_capacity);
    if (!new_stack)
      return false;
...
void gen_until(gen_ctx *ctx, csq_node *node) {
  if (!node || node->type != NODE_UNTIL)
    return;

  char *start_label = label_gen_new(ctx->labels, "until");
  char *end_label = label_gen_new(ctx->labels, "enduntil");
  ...
  emit_label(ctx->emit, start_label);
  emit_indent(ctx->emit);
  gen_expression(ctx, node->data.until_stmt.condition);
  inst_cmp_reg_imm(ctx->emit, REG_EAX, 0);
  inst_jne_label(ctx->emit, end_label);

  emit_indent(ctx->emit);
  gen_statement(ctx, node->data.until_stmt.body);
  emit_dedent(ctx->emit);
  inst_jmp_label(ctx->emit, start_label);
...
void gen_for(gen_ctx *ctx, csq_node *node) {
  if (!node || node->type != NODE_FOR)
    return;

  char *start_label = label_gen_new(ctx->labels, "for");
  char *end_label = label_gen_new(ctx->labels, "endfor");

  gen_push_loop(ctx, start_label, end_label);

  emit_dedent(ctx->emit);

  node_type iterable_type = node->data.for_stmt.iterable->type;

  if (iterable_type == NODE_RANGE) {
    const char *vname = node->data.for_stmt.var->data.identifier.name;
    var_info *idx = var_table_add(ctx->vars, vname, type_int());

    csq_node *range = node->data.for_stmt.iterable;
    csq_node *start = range->data.range.start;
    csq_node *end = range->data.range.end;

    gen_expression(ctx, start);
    emit_line(ctx->emit, "movl %%eax, %d(%%ebp)", idx->offset);

    emit_label(ctx->emit, start_label);
    emit_indent(ctx->emit);

    gen_statement(ctx, node->data.for_stmt.body);

    emit_line(ctx->emit, "movl %d(%%ebp), %%eax", idx->offset);
    gen_expression(ctx, end);
    emit_line(ctx->emit, "movl %%eax, %%ebx");
    emit_line(ctx->emit, "movl %d(%%ebp), %%eax", idx->offset);
    inst_cmp_reg_reg(ctx->emit, REG_EAX, REG_EBX);
    inst_jge_label(ctx->emit, end_label);

    emit_line(ctx->emit, "movl %d(%%ebp), %%eax", idx->offset);
    inst_inc_reg(ctx->emit, REG_EAX);
    emit_line(ctx->emit, "movl %%eax, %d(%%ebp)", idx->offset);
    ...
    if (ctx->diag) {
      ...
      LocationID loc_id = diag_location_add(loc_mgr, ctx->source_path,
                                            node->line, node->column, 0);
      diag_report_fmt(ctx->diag, ctx->codegen_error_id, loc_id,
                      "unknown iterable type '%s'",
...
void gen_continue(gen_ctx *ctx, csq_node *node) {
  if (!node || node->type != NODE_CONTINUE)
    return;
